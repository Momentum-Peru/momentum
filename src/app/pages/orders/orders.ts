...
export class OrdersPage {
  ...
  newItem() {
    this.editing.set({ clientId: '', clientName: '', number: '', type: 'purchase' });
  ...
  save() {
    ...
    if (!item) return;
    const req = item._id
      ? this.http.patch<OrderItem>(`${this.baseUrl}/orders/${item._id}`, item)
      : this.http.post<OrderItem>(`${this.baseUrl}/orders`, item);
    req.subscribe(() => {
      this.closeDialog();
      this.load();
    });
  ...
  remove(item: OrderItem) {
    this.http.delete(`${this.baseUrl}/orders/${item._id}`).subscribe(() => this.load());
  }

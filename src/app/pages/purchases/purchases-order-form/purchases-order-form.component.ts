...
import { CreatePurchaseOrderRequest, PurchaseOrderLine } from '../../../shared/interfaces/purchase.interface';
...
@Component({
    ...
    imports: [
        ...
        Select, CheckboxModule, CardModule, ToastModule, TooltipModule,
...
export class PurchasesOrderFormComponent implements OnInit {
    ...
    addLine() {
        ...
        this.orderForm.set({
            ...cur,
            lines: [...cur.lines, { requirementItemIndex: cur.lines.length, description: '', quantity: 1, unit: 'UND', unitPrice: 0, includesIgv: true, discount: 0 }]
        });
    ...
    save() {
        ...
        if (this.isEditMode()) {
            ...
            this.ordersApi.create(payload).subscribe({
                next: () => {
                    this.messageService.add({ severity: 'success', summary: 'Éxito', detail: 'Orden de compra generada' });
                    setTimeout(() => this.goBack(), 1500);
                },
                error: (error) => {
                    this.messageService.add({ severity: 'error', summary: 'Error', detail: error.error?.message || 'Error al generar la orden' });
                }
            });
        }
    }

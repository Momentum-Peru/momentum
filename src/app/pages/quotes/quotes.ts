...
export class QuotesPage {
  ...
  newItem() {
    this.editing.set({ clientId: '', clientName: '', items: [] });
  ...
  save() {
    ...
    if (!item) return;
    const req = item._id
      ? this.http.patch<Quote>(`${this.baseUrl}/quotes/${item._id}`, item)
      : this.http.post<Quote>(`${this.baseUrl}/quotes`, item);
    req.subscribe(() => {
      this.closeDialog();
      this.load();
    });
  ...
  remove(item: Quote) {
    this.http.delete(`${this.baseUrl}/quotes/${item._id}`).subscribe(() => this.load());
  }
  ...
  setStatus(item: Quote, status: string) {
    this.http
      .post(`${this.baseUrl}/quotes/${item._id}/status/${status}`, {})
      .subscribe(() => this.load());
  }
  openPdf(item: Quote) {
    window.open(`${this.baseUrl}/quotes/${item._id}/pdf`, '_blank');
  }
}

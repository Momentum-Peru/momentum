...
import { Accionable, AccionableEstado, CalendarDay, CalendarResponse, Fi, UpdateFiRequest } from '../../shared/interfaces/fi';
...
@Component({
	...
	imports: [CommonModule, RouterModule, FormsModule, Button, InputText, Textarea, ToggleSwitch, DatePicker, Dialog, ConfirmDialogModule],
...
export class FiDetailPage implements OnInit {
	...
	form: { titulo: string; atravesar: string; isActive: boolean } = { titulo: '', atravesar: '', isActive: true };
	planDesc = '';
	startDate?: Date;
	endDate?: Date;
	dateRange: Date[] = [];
	...
	save(): void {
		...
		this.api.update(this.fiId, payload).subscribe((updated) => {
			this.fi.set(updated);
			this.reloadCalendar();
		});
	}
	...
	saveAccionable(): void {
		...
		if (editing) {
			this.api.updateActionable(this.fiId, editing._id, { fecha, descripcion: this.accionableDesc }).subscribe(() => {
	...
	deleteAccionable(item: Accionable): void {
		this.api.deleteActionable(this.fiId, item._id).subscribe(() => this.reloadCalendar());
	}

	toggleEstado(item: Accionable): void {
		const next: AccionableEstado = item.estado === 'cumplido' ? 'pendiente' : 'cumplido';
		this.api.updateActionableEstado(this.fiId, item._id, next).subscribe(() => this.reloadCalendar());
	}

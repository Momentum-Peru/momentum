...
export class EmployeeFormPage implements OnInit {
    ...
    contractTypes = [
        ...
        { label: 'Sujeto a Modalidad: Inicio de Actividad', value: 'Sujeto a Modalidad: Inicio de Actividad' },
        { label: 'Sujeto a Modalidad: Necesidad de Mercado', value: 'Sujeto a Modalidad: Necesidad de Mercado' },
        { label: 'Sujeto a Modalidad: Obra determinada', value: 'Sujeto a Modalidad: Obra determinada' }
    ];
    ...
    shoeSizes = Array.from({ length: 10 }, (_, i) => ({ label: (36 + i).toString(), value: (36 + i).toString() }));
    ...
    bloodTypes = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(s => ({ label: s, value: s }));
        ...
        this.employeesApi.getById(this.employeeId()!).subscribe({
            next: (data) => {
                const mapped = {
                    ...
                    workShiftId: typeof data.workShiftId === 'string' ? data.workShiftId : (data.workShiftId as any)?._id,
                    ...
                    fechaVencimientoSuspension: data.fechaVencimientoSuspension ? new Date(data.fechaVencimientoSuspension) : null,
                    examenMedico: {
                        ...(data.examenMedico || {}),
                        fechaVencimiento: data.examenMedico?.fechaVencimiento ? new Date(data.examenMedico.fechaVencimiento) : null
                    },
                    sctr: {
                        ...(data.sctr || {}),
                        fechaVencimiento: data.sctr?.fechaVencimiento ? new Date(data.sctr.fechaVencimiento) : null
                    },
                    licenciaConducir: {
                        ...(data.licenciaConducir || {}),
                        fechaVencimiento: data.licenciaConducir?.fechaVencimiento ? new Date(data.licenciaConducir.fechaVencimiento) : null
                    },
                    antecedentes: {
                        ...(data.antecedentes || {}),
                        fechaEmision: data.antecedentes?.fechaEmision ? new Date(data.antecedentes.fechaEmision) : null
                    },
    ...
    isStep3Valid(): boolean {
        const val = this.e();
        return !!(val.tipoEmpleado && val.cargo && (val.sueldoBasico !== undefined && val.sueldoBasico !== null));
    }
    ...
    onContratosSelect(event: any) {
        ...
        this.selectedFiles['contratos'] = [...this.selectedFiles['contratos'], ...Array.from(files) as File[]];
    }

    onAntecedentesSelect(event: any) {
        const files = event.files || (event.target && event.target.files) || [];
        this.selectedFiles['antecedentesPoliciales'] = [...this.selectedFiles['antecedentesPoliciales'], ...Array.from(files) as File[]];
    }

    onConstanciaSelect(event: any) {
        const files = event.files || (event.target && event.target.files) || [];
        this.selectedFiles['constanciaSuspensionRenta'] = [...this.selectedFiles['constanciaSuspensionRenta'], ...Array.from(files) as File[]];
    }
    ...
    removeUrl(field: string, url: string) {
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.e.set({ ...current });
                this.messageService.add({ severity: 'info', summary: 'Info', detail: 'Archivo marcado para eliminación' });
            }
        });
    }
    ...
    save() {
        ...
        if (data.fechaInicioLabores instanceof Date) data.fechaInicioLabores = data.fechaInicioLabores.toISOString(); // @ts-ignore
        else if (typeof data.fechaInicioLabores === 'string' && data.fechaInicioLabores) data.fechaInicioLabores = new Date(data.fechaInicioLabores).toISOString();

        if (data.fechaFinContrato instanceof Date) data.fechaFinContrato = data.fechaFinContrato.toISOString(); // @ts-ignore
        else if (typeof data.fechaFinContrato === 'string' && data.fechaFinContrato) data.fechaFinContrato = new Date(data.fechaFinContrato).toISOString();

        if (data.fechaVencimientoSuspension instanceof Date) data.fechaVencimientoSuspension = data.fechaVencimientoSuspension.toISOString(); // @ts-ignore
        else if (typeof data.fechaVencimientoSuspension === 'string' && data.fechaVencimientoSuspension) data.fechaVencimientoSuspension = new Date(data.fechaVencimientoSuspension).toISOString();

        if (data.examenMedico?.fechaVencimiento instanceof Date) data.examenMedico.fechaVencimiento = data.examenMedico.fechaVencimiento.toISOString();
        if (data.sctr?.fechaVencimiento instanceof Date) data.sctr.fechaVencimiento = data.sctr.fechaVencimiento.toISOString();
        if (data.licenciaConducir?.fechaVencimiento instanceof Date) data.licenciaConducir.fechaVencimiento = data.licenciaConducir.fechaVencimiento.toISOString();
        if (data.antecedentes?.fechaEmision instanceof Date) data.antecedentes.fechaEmision = data.antecedentes.fechaEmision.toISOString();

        if (data.hijos) {
            data.hijos = data.hijos.map((h: any) => ({
                ...h,
                fechaNacimiento: h.fechaNacimiento instanceof Date ? h.fechaNacimiento.toISOString() : h.fechaNacimiento
            }));
        }
        ...
        action.subscribe({
            ...
            error: () => {
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Error al guardar el empleado' });
                this.loading.set(false);
            }
        });
    }
    ...
    private uploadSequentially(employeeId: string) {
        ...
        if (tasks.length === 0) {
            this.messageService.add({ severity: 'success', summary: 'Éxito', detail: 'Empleado guardado correctamente' });
            this.router.navigate(['/employees']);
            return;
        }
        ...
        forkJoin(tasks).subscribe({
            next: () => {
                this.messageService.add({ severity: 'success', summary: 'Éxito', detail: 'Empleado y documentos guardados correctamente' });
                this.router.navigate(['/employees']);
            },
            error: () => {
                this.messageService.add({ severity: 'warn', summary: 'Advertencia', detail: 'Empleado guardado, pero hubo errores al subir algunos documentos' });
                this.router.navigate(['/employees']);
            }
        });
    }

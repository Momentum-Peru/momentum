...
interface TdrItem { _id?: string; clientId: string; type: 'client' | 'tecmeing'; summary?: string; approved?: boolean; documents?: string[]; }

@Component({
    selector: 'app-tdrs',
    standalone: true,
    imports: [CommonModule, HttpClientModule, FormsModule, InputTextModule, ButtonModule, TableModule, DialogModule, SelectButtonModule, SelectModule],
...
export class TdrsPage {
    ...
    tdrTypeOptions = [{ label: 'Cliente', value: 'client' }, { label: 'Tecmeing', value: 'tecmeing' }];
    ...
    newItem() { this.editing.set({ clientId: '', type: 'client' }); this.showDialog.set(true); }
    editItem(item: TdrItem) { this.editing.set({ ...item }); this.showDialog.set(true); }
    ...
    save() {
        ...
        const req = item._id ? this.http.patch<TdrItem>(`${this.baseUrl}/tdrs/${item._id}`, item) : this.http.post<TdrItem>(`${this.baseUrl}/tdrs`, item);
        req.subscribe(() => { this.closeDialog(); this.load(); });
    }
    remove(item: TdrItem) { this.http.delete(`${this.baseUrl}/tdrs/${item._id}`).subscribe(() => this.load()); }
    approve(item: TdrItem) { this.http.post(`${this.baseUrl}/tdrs/${item._id}/approve`, {}).subscribe(() => this.load()); }

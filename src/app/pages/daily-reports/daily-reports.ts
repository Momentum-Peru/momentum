...
import { DailyExpense, Purchase, Observation } from '../../shared/interfaces/daily-report.interface';
...
export class DailyExpensesPage implements OnInit {
    ...
    removeObservationDocument(observationIndex: number, documentUrl: string): void {
        ...
        this.dailyExpensesApi.deleteObservationDocument(editing._id, observationIndex, documentUrl).subscribe({
            next: () => {
                editing.observations[observationIndex].documents = editing.observations[observationIndex].documents.filter(url => url !== documentUrl);
                this.editing.set({ ...editing });
            },
            error: (error) => {
    ...
    removePurchaseDocument(purchaseIndex: number, documentUrl: string): void {
        ...
        this.dailyExpensesApi.deletePurchaseDocument(editing._id, purchaseIndex, documentUrl).subscribe({
            next: () => {
                editing.purchases[purchaseIndex].documents = editing.purchases[purchaseIndex].documents.filter(url => url !== documentUrl);
                this.editing.set({ ...editing });
            },
            error: (error) => {
    ...
    getFileTypeColor(url: string): string {
        ...
        switch (extension) {
            ...
            case 'jpeg':
            case 'png':
            case 'gif': return 'text-green-600';
            case 'doc':

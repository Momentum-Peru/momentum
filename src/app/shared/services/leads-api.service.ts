...
export class LeadsApiService {
  ...
  convertToClient(id: string, request: ConvertLeadToClientRequest): Observable<Lead> {
    return this.http.post<Lead>(`${this.baseUrl}/${id}/convert-to-client/${request.clientId}`, {});
  }
  ...
  getPresignedUrlForAudio(
    ...
    return this.http.post<PresignedUrlResponse>(
      `${this.baseUrl}/${id}/audio/presigned-url`,
      request,
    );
  }

  /**
   * Obtiene una URL presignada genérica para subir audio a S3 (sin lead asociado)
   */
  getPresignedUrlForAudioGlobal(request: PresignedUrlRequest): Observable<PresignedUrlResponse> {
    return this.http.post<PresignedUrlResponse>(`${this.baseUrl}/audio/presigned-url`, request);
  }
  ...
  /**
   * Analiza un audio sin asociarlo obligatoriamente a un lead
   */
  analyzeAudioGlobal(request: AudioAnalysisRequest): Observable<AudioAnalysisResponse> {
    return this.http.post<AudioAnalysisResponse>(`${this.baseUrl}/analyze-audio`, request);
  }
  ...
  generatePhotoPresignedUrl(
    ...
    return this.http.post<PresignedUrlResponse>(`${this.baseUrl}/${leadId}/photo/presigned-url`, {
      fileName,
      contentType,

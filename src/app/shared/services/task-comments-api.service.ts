...
export class TaskCommentsApiService {
     ...
     */
    createComment(commentData: CreateTaskCommentRequest): Observable<TaskComment> {
        this.setLoading(true);
        this.setError(null);

        return this.http.post<TaskComment>(`${this.baseUrl}/tasks/${commentData.taskId}/comments`, {
     ...
     */
    updateComment(taskId: string, commentId: string, commentData: UpdateTaskCommentRequest): Observable<TaskComment> {
        this.setLoading(true);
        this.setError(null);

        return this.http.put<TaskComment>(`${this.baseUrl}/tasks/${taskId}/comments/${commentId}`, commentData)
            .pipe(
                tap(() => this.setLoading(false)),
                tap({ error: (err) => this.handleError(err) })
            );
    ...
    uploadCommentFile(taskId: string, commentId: string, file: File): Observable<TaskFile> {
        ...
        return this.http.post<TaskFile>(`${this.baseUrl}/tasks/${taskId}/files`, formData)
            .pipe(
                tap(() => this.setLoading(false)),
                tap({ error: (err) => this.handleError(err) })
            );
